...
public class ArtificialImageGeneratorStandard implements Runnable {
	...
	public ArtificialImageGeneratorStandard(final File folder,
	        ...
	        final boolean hasPoissonGen, final boolean hasGenerateLog,
	...
	public void run() {
		...
		this.updateGUI("Generating folders...");
		...
		for (int k = 0; k < this.signalPeakValues.size(); k++) {
			...
			for (int i = 0; i < this.numOfDatasets; i++) {
				this.updateGUI("Generating dataset s="
				        + this.currentSignalPeakValue + " " + k + "/"
				        + this.signalPeakValues.size() + " " + i + "/"
				        + this.numOfDatasets);
